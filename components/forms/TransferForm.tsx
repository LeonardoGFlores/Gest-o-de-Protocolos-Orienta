...
export const TransferForm: React.FC<FormProps> = ({ onSave, categories, farms, farm, initialData, onCancelEdit }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <h3 className="text-xl font-semibold text-brand-green mb-4">
        {initialData ? 'Editando Lançamento de Transferência' : 'Adicionar Lançamento de Transferência'}
      </h3>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <Input label="Data" type="date" name="date" value={formData.date} onChange={handleChange} required />
        <Input label="Pasto de Origem" name="originPasture" value={formData.originPasture} onChange={handleChange} />
        <Select label="Fazenda de Destino" name="destinationFarmId" value={formData.destinationFarmId} onChange={handleChange}>
           <option value="">Selecione</option>
           {farms.filter(f => f.id !== farm?.id).map(f => <option key={f.id} value={f.id}>{f.name} - {f.city}</option>)}
        </Select>
        <Input label="Pasto de Destino" name="destinationPasture" value={formData.destinationPasture} onChange={handleChange} />
        ...
        <Input label="Valor do Kg gerencial" type="number" name="managerialKgValue" value={formData.managerialKgValue} onChange={handleChange} />
        <Input label="Valor total" type="number" name="totalValue" value={formData.totalValue} onChange={handleChange} />
        <Input label="Atividade (= Lote)" name="activity" value={formData.activity} onChange={handleChange} />
      </div>
